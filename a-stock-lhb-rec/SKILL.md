...
- **模型择优（LR vs GBDT）**：训练时在**同一时间外测试集**上比较逻辑回归与梯度提升树，GBDT 明显更优且样本充足（≥300）时自动采用 GBDT 作生产模型，否则回落 LR（稀疏样本下 LR 更稳），模型类型记入 `model_kind`。
- **暴雷一票否决**：推荐输出阶段对 ST/退市/立案调查/违规处罚/业绩预亏等风险股强制剔除（即使模型概率高也不纳入观察候选）。
...
- 定时自动化（可选）：每个交易日 17:00 后触发 `daily`；该命令**自身已内嵌「生成报告后自动自我学习」**，无需另行配置 cron 任务
...
4. `report` —— 刷新 Markdown 报告（含最新进化轨迹与验证记录）
5. `optimize`（**自动**）—— 生成报告后自动自我学习：把最新实际结果补入训练集、重训、版本决策（时间外 AUC 不劣则 +1），无需额外定时任务
...
py scripts/run.py report                 # 重新生成报告(生成后自动触发自我学习)
py scripts/run.py train                   # 仅重训(不抓数据)
```

## 自进化机制（为什么越来越准）

> `daily` / `init` / `report` 每次生成报告后都会**自动**运行 `optimize` 完成自我学习（记录当日数据 → 重训 → 版本决策），全程无需手动或额外 cron 定时任务。
...
- **主口径**：TopN 概率排序（含 P 值）＋当日市场强弱门控。「纳入观察」仅辅助标注（`P≥30%` 且 `前3日透支<38%` 且 非纯机构主买），不再用「主线+净买比≥0.55」等硬门槛筛掉高 P 股票（90 日 PIT 回溯证明该框命中 27.3% 低于纯 Top20 的 31.2%）。
- **回避**：纯机构主买 / 前3日透支≥38% / 高位连板≥5 / 科创创业涨停上榜（已充分定价）。
- **风控**：T+1 不涨停即减仓，跌破上榜日收盘止损；弱市（当日上榜涨停占比处于历史低分位）自动降级并提示勿追高。
...
- **v1.3.0（2026-09-04）· 基于 90 交易日 PIT 回溯诊断的模型优化**
  - 去共线：训练特征移除 `CHANGE_RATE`（与 SEAL_STRENGTH r=0.94）、`THEME_MAIN`（与 THEME_STRENGTH r=0.69）、`LOW_XI`（与 REASON_跌幅偏离 完全重复），消除系数互相抵消导致的方向失真。
  - 方向先验：按真实 T+3 单因子 AUC 锁定 `SEAL_STRENGTH/CONSEC_ZT/LB_FLAG/NET_BUY_RATIO/IS_LIMIT_UP/THEME_STRENGTH` 为正、`BUYER_MEAN_SCORE/HAS_INST/SELLER_MEAN_SCORE/TURNOVERRATE` 为负（符号冲突归零）；`BUYER_TOP_SCORE` 权重收缩至 30%（原 -0.187 过强）。
  - 决策口径：主交付从「纳入观察手工框」改为「**TopN 概率排序**」为主，「纳入观察」仅作辅助标注（放宽主线+净买比硬门槛）。
  - 市场门控：`recommend` 新增当日市场强弱判定（按上榜涨停占比的历史分位），弱市自动降级提示；报告主监测口径改为「滚动 Top20（近似 PIT 平均命中）」。
  - 效果：滚动 Top20 命中 33.8% → 35.0%（同一 4-fold walk-forward 口径），尾部 Top20 45% → 50%。
- **v1.3.1（2026-09-04）· 修复特征口径不一致（训练/评估/生产统一 one-hot）**
  - 缺陷：`train.py` 生产模型含 `REASON_*` 上榜原因 one-hot 特征（29 维），但时间外评估与 walk-forward 却用未 one-hot 的 21 维数值特征，导致 AUC/Top20 口径错位（v3~v5 的 `top20_wf=40%`、`top20=55%` 为虚高值）。
  - 修复：`main()` 统一对 `df` 做 one-hot，`evaluate/walk_forward/train_full` 全部使用同一 29 维特征；`_common.prep_features` 兼容已 one-hot 的 DataFrame。
  - 效果（真实口径）：时间外 AUC 0.671 → 0.694，滚动 Top20 35.0%、尾部 Top20 50.0%。
- **v1.2.0**：因果特征（消除未来泄漏）+ LR/GBDT 同集择优 + walk-forward 双门控。
- **v1.1.0**：席位画像、紫阳东路专项、暴雷一票否决。
