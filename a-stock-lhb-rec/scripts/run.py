...
"""龙虎榜 T+3 涨停推荐 · 统一入口
子命令:
  init                  首次全量构建(近90天数据 -> 画像 -> 特征 -> 训练 -> 推荐 -> 报告)
  daily [DATE]          每日收盘后主流程: 抓最新龙虎榜 -> 推荐 -> 验证前几日 -> 自我进化重训
  recommend [DATE]     仅对指定日(缺省=最近已发布交易日)生成 T+3 推荐候选
  verify [DATE]        验证历史推荐实际命中(缺省=最近 recommend 文件)
  optimize             自我进化: 补最新数据 -> 重训 -> 版本决策
  train                仅重训(不抓数据)
  report [DATE]        生成 HTML 报告
示例:
  python run.py init
  python run.py daily
...
def cmd_daily(date=None):
    ...
    # 4) 自我进化: 补最新实际结果并重训
    try:
        T2 = __import__("optimize")
        T2.main()
    except Exception as e:
        print(f"  optimize 跳过: {e}")
    RP.main(date)
    print("=== daily 完成 ===")
