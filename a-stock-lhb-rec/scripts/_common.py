...
运行时目录(RUNTIME)存放所有可写产物(采集数据/模型/进化轨迹/报告)。
RUNTIME 定位优先级：
  1) 环境变量 LHB_RUNTIME（最明确，可指向任意工程目录）
  2) 否则 = 「当前会话/工作目录(os.getcwd())」下新建 <skill>_runtime/
     —— 让产物落在调用本技能时所处的工程目录，而不埋在技能安装目录里(不改 SKILL 本体)。
RUNTIME/
...
# 会话产物根：优先 LHB_RUNTIME，否则收敛到「当前工作目录」下的 <技能名>/ 子目录，避免写回技能安装处
if os.environ.get("LHB_RUNTIME"):
    RUNTIME = os.environ["LHB_RUNTIME"]
else:
    RUNTIME = os.path.join(os.getcwd(), "a-stock-lhb-rec")
DATA = os.path.join(RUNTIME, "data")
MODEL_DIR = os.path.join(RUNTIME, "model")
# 最终报告（Markdown，带日期）输出根 = 当前会话/工作目录根，不放进技能名子目录
REPORT_ROOT = os.environ.get("LHB_OUT") or os.getcwd()
