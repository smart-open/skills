...
16. 卫语句式空检查，防御式编程；**禁止魔法值**——数字/字符串先定义常量再引用（黄山版增补决议）；魔法数字必须校验（如批量上限500）
...
> 条目 24-28 为《阿里巴巴Java开发手册》黄山版（1.7.1）蒸馏增补，2026-09-11 用户确认入库。#24 为并发纪律**合并条目**（原 #24-27 四条合并，2026-09-11 用户确认，落实 coding 层软上限预警）。

24. **并发纪律（业界增补决议，黄山版蒸馏）**：①线程池一律显式 `new ThreadPoolExecutor(...)` 创建，**禁用 `Executors` 快捷工厂**——Cached/Scheduled 可创建 `Integer.MAX_VALUE` 线程致 OOM；②`SimpleDateFormat` 线程不安全，禁 static 共享，新代码统一 `DateTimeFormatter`；③`CountDownLatch` 异步转同步时 `countDown()` 必须保证执行——finally 兜底，线程内异常不得吞掉 countDown；④悲观锁遵循"**一锁、二判、三更新、四释放**"顺序
25. 集合初始化指定容量（`new ArrayList<>(expectedSize)`）；Map 遍历优先 `entrySet` 一次取 KV，不用 keySet 二次 get（业界增补）
26. `error` 日志级别克制——仅记系统逻辑错误/重要异常，避免频繁报警致麻痹；占位符拼接见条目4（黄山版增补决议）
27. 文件上传必须**大小 + 类型双重严格校验**（类型白名单），杜绝任意文件落盘（黄山版增补决议）
28. 常量类按功能域拆分维护（缓存常量/系统配置分开），不用单一巨型常量类（业界增补）
...
> 2026-09-11 对齐《阿里巴巴Java开发手册》黄山版（1.7.1）：唯一索引命名 `uniq_` → 手册标准 `uk_`（用户拍板按手册），补主键 `pk_`，并增补字段类型/逻辑删除/索引使用/SQL安全规约（用户确认入库）。

- **存储策略（决议）**：数据库仅作存储与查询——**不用函数、存储过程、外键等数据库侧逻辑**，业务逻辑全部在应用层实现；不建外键约束，完整性校验交由应用层
- **索引命名（决议，2026-09-11 对齐黄山版）**：主键 `pk_`、唯一 `uk_`、普通 `idx_` 开头（`pk_字段名` / `uk_字段名` / `idx_字段名`）
- **表命名（决议）**：业务表 = **领域简写前缀 + 表名**，小写下划线分词、按领域隔离——如文档领域 `doc_log_record`（文档日志记录），避免全库扁平堆砌
- **字段类型（黄山版增补决议）**：小数一律 `decimal`，**禁用 float/double**（精度丢失）；表/字段名禁用 MySQL 保留字（desc/range/match/delayed 等）
- **删除策略（黄山版增补决议）**：**禁物理删除，一律逻辑删除**（删除标记字段），数据可追溯可恢复
- **索引使用（黄山版增补决议）**：联合索引遵守**最左前缀**；查询条件避免隐式类型转换、函数包裹列等索引失效写法
- **时间字段（黄山版增补建议）**：时间类型优先 `timestamp`，全链路时区统一
- **SQL 安全（黄山版增补决议）**：SQL 参数**一律参数绑定**（MyBatis `#{}` / PreparedStatement），**禁止字符串拼接 SQL**（防注入）
