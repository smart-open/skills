...
> 一套面向 AI 多媒体创作的 Skills 集合 — 涵盖文生图、图生图、文生视频、图生视频、AI 音乐、跨朝代婚礼电影、脱口秀视频、技术文章、学习手册、企业门户、A 股量化分析与技能数据抓取，以及战略思考与决策。
...
本工程聚合了 **21 个独立 Skills**（外加 8 个技能数据抓取子技能），覆盖 AI 创作与数据分析的核心场景：**图片生成、视频生成、音乐创作、音乐下载、跨时空婚礼电影、脱口秀视频、技术文章、学习手册、企业门户网站、全网深度画像、A 股操盘分析、首板洗盘选股、龙虎榜涨停推荐、一阳指量化扫描、软件开发流程编排、个人开发分身、战略思考与决策、技能数据抓取**。
...
| `strategic-advisor` | 战略顾问技能（七步深度对话流程，双文件输出，零依赖架构） | 无（纯规范型） | 无 | 约 20–40 分钟（包含最终审查） |
| `dev-twin` | 个人开发分身引擎（画像库与引擎解耦、多级覆盖，按画像自主开发：定位→路由→决策分级→学习闭环） | 无（纯规范型） | 画像库（内置默认 + 外部 `persona-*` 覆盖） | 与开发任务等同 |
...
│
├── strategic-advisor/                 # 战略顾问技能（七步深度对话流程）
│   └── SKILL.md
│
├── dev-twin/                          # 个人开发分身引擎（画像库内置默认+多级覆盖）
│   ├── SKILL.md                       # 引擎：画像定位→任务路由→决策分级→学习闭环
│   ├── references/                    # 复盘规程（仅复盘时加载）
│   └── persona-zhutianwei/            # 内置默认画像库（朱天伟，可替换为自己的）
│       ├── profile-*.md               # 分层画像：core / coding / architecture / workflow / tools / polyglot
│       └── corpus/                    # 学习语料（HITL 进化闭环）
│
├── docs/                              # 战略调研报告与行动计划文档
...
**核心能力：** 市场全景（情绪温度计 + 连板梯队 + 主线题材）、近一月板块轮动、资金轮动四象限、个股六维双权重诊断，以及**次日轮动主线推荐**（四维打分 + 自学习闭环）。
...
> 报告输出到当前工程目录（Markdown，带日期），`A_STOCK_OUT` 可覆盖。每次运行自动触发轮动推荐验证 + 权重自学习（无需定时任务）。
...
#### 20. strategic-advisor — 战略顾问技能

将AI从传统的"听指挥工具人"转变为"主动思考的战略顾问"，通过**七步深度对话流程**帮助用户将模糊想法转化为清晰的战略调研报告和可执行行动计划。

**核心特色：**
- **🎭 角色互换模式**：从"你问AI答"到"AI问你答"，AI主动引导思考
- **🧅 五层渐进式提问**：现状探查→问题拆解→目标对齐→路径发现→风险识别
- **📊 双文件输出**：战略调研报告（📊现状分析→🎯目标定义→💡关键发现→🚀战略建议→✅下一步行动）+ 详细行动计划（📅分步实施→📋具体行动→🏆成功标准→🎒资源需求→🛡️风险应对）
- **✅ 七步质量保证**：角色声明→五层对话→信息整合→报告生成→计划制定→移交选择→整体审查和逻辑验证
- **🔧 零依赖架构**：纯规范型技能，完全使用内置工具（AskUserQuestion、TodoWrite、Write、Read、NotifyUser）

**七步流程：**
1. **角色声明** - 建立正确期望，切换到战略顾问角色
2. **五层深度对话** - 每层5个问题，覆盖现状、问题、目标、路径、风险（约15-30分钟）
3. **整理关键发现** - 信息整合和SWOT分析
4. **生成调研报告** - 结构化输出，Markdown格式+图标
5. **制定行动计划** - 详细实施步骤和可执行清单
6. **移交和选择** - 选择执行方式（AI执行/移交给其他技能/继续深入）
7. **整体审查和逻辑验证** - 六维度质量检查（逻辑严谨性、报告可执行性、计划可操作性、格式规范性、整体质量、最终结论）
...
**输出规范：**
- **文件格式**：标准Markdown (.md)，1-3级标题前增加合适的图标
- **输出目录**：当前会话根目录 (`<cwd>`)
...
> **产物落点**：两份文件输出到当前会话根目录（`<cwd>`），不写回技能安装目录。
...
#### 20. dev-twin — 个人开发分身引擎

外挂个人画像库的 AI 分身引擎：一切决策、代码风格、流程节奏来自**画像库**（`persona-*` 目录与引擎解耦，替换画像目录即切换分身）。按画像自主完成需求分析、架构设计、编码开发、测试审查。

**核心机制：**

- 画像定位（5级优先级：用户指定 / `PERSONA_DIR` / cwd / skill同级 / skill内置默认，外部优先于内置，可部署任意目录）
- 分层按需加载（任务路由表，同时激活画像层 ≤3，token 可控）
- 决策三级分级（L1自主执行留痕 / L2请示决策 / L3强制确认）
- HITL 学习闭环（纠正→语料→攒10项→复盘确认→入画像，画像永不污染）
- 决策全程留痕（`coding#12 / core#D2 / 决议日期` 三种引用格式）
- 执行九纪律（决策留痕 / token纪律 / 风格对齐 / 审查者视角自查 / 阶段交接摘要 / 事实自查决策请示 等）

内置默认画像库 `dev-twin/persona-zhutianwei/`（v0.6，6分层+语料库：core / coding / architecture / workflow / tools / polyglot多语言全栈，约200条画像条目，来源：简历+个人经验+10W行工程代码分析+业界最佳实践双重校准+两轮外部skill蒸馏：ECC/mattpocock）。

> 适用：按个人习惯/画像执行开发任务、生成符合个人风格的代码、以"分身"身份推进完整开发流程。他人复制画像库结构即可生成自己的分身。
...
| 模糊想法→战略分析→行动计划 | `strategic-advisor` | 七步深度对话 + 五层渐进式提问 + 双文件输出（战略调研报告+行动计划）+ 零依赖架构 |
