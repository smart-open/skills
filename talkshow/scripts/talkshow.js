#!/usr/bin/env node
/**
 * talkshow.js — 主入口 CLI，流水线编排，状态管理
 *
 * 用法：
 *   node talkshow.js --script "脱口秀段子" --output "output.mp4"
 *   node talkshow.js --script-file script.txt --ratio 16:9
 *   node talkshow.js --resume --work-dir work/20260716-xxxx
 */
...
// 产物（work/ 会话目录）默认落到「当前工作目录/talkshow/」，不写回技能安装目录
const WORK_DIR = process.env.TALKSHOW_WORK || path.join(process.cwd(), "talkshow");
const FPS = 24;
...
async function main() {
  ...
  // 环境检查
  console.log("=== Talkshow Generator ===");
  findEdgeTts();
  findFFmpeg();
  ...
  if (args.resume && args.workDir) {
    ...
    state = {
      ...
      scenes: {},
      errors: {},
      outputPath: args.output || path.join(workDir, "final", `talkshow_${sessionId}.mp4`),
    };
