 ...
 * - Wav2Lip 仓库（通过环境变量 WAV2LIP_DIR 指定）
 * - PyTorch 库（通过环境变量 PYTHON_LIBS 指定）
 */
...
const WAV2LIP_DIR = process.env.WAV2LIP_DIR || path.join(os.homedir(), "Wav2Lip");
const PYTHON_LIBS = process.env.PYTHON_LIBS || "";
...
function applyLipSync(videoPath, audioPath, outputPath, options = {}) {
  ...
  const env = {
    ...process.env,
    PYTHONPATH: [PYTHON_LIBS, WAV2LIP_DIR].filter(Boolean).join(";"),
