...
- **PyTorch CPU 版**：安装到独立目录（如 `<你的目录>/libs`），通过 `PYTHONPATH` 注入，不污染全局环境
- **Wav2Lip 仓库**：克隆到 `<你的目录>/Wav2Lip`，需包含 `inference.py` 与 `checkpoints/`
- **模型权重**：
...
```powershell
# 1. 克隆 Wav2Lip 仓库
git clone https://github.com/Rudrabha/Wav2Lip <你的目录>/Wav2Lip

# 2. 下载预训练权重到 checkpoints/ 目录
#    wav2lip_gan.pth（推荐）、wav2lip.pth

# 3. 安装 PyTorch CPU 版（示例：安装到独立 libs 目录）
pip install torch torchvision --index-url https://download.pytorch.org/whl/cpu --target <你的目录>/libs
```
...
```powershell
node talkshow.js --script-file joke.txt --api-key "your-key"
```

### 横版视频 + 字幕

```powershell
node talkshow.js --script "段子内容" --ratio 16:9 --add-subtitles --api-key "your-key"
```
...
```powershell
# 中途中断后恢复
node talkshow.js --resume --work-dir "work/20260716-143022-a1b2" --api-key "your-key"
```
...
node talkshow.js --resume --work-dir "work/20260716-143022-a1b2" --regenerate 1 --api-key "your-key"
```
...
└── 20260716-143022-a1b2/
    ...
    └── final/                  # 最终输出
        └── talkshow_20260716-143022-a1b2.mp4
```
...
| 文件 | 职责 |
|------|------|
| `talkshow.js` | 主入口 CLI，4阶段流水线编排，状态管理，断点续跑 |
...
| "Wav2Lip checkpoint not found" | 下载 `wav2lip_gan.pth` 到 `<你的目录>/Wav2Lip/checkpoints/` 目录 |
| torch DLL 加载失败 / "DLL load failed" | PyTorch CPU 版依赖 VC++ 运行库，安装 [Visual C++ Redistributable](https://aka.ms/vs/17/release/vc_redist.x64.exe)；或确认 PyTorch 安装在 `<你的目录>/libs` 且 `PYTHONPATH` 已注入 |
